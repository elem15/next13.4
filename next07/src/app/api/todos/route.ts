...
export async function POST(request: Request) {
  ...
  if(!userId || !title) return NextResponse.json({ "message": "UserId and Title are required" })
  ...
  await fetch(address, {
    ...
    body: JSON.stringify({userId, title, completed: true})
  })
    return NextResponse.json({"message": `Post with title ${title} was created`})
}
